...
int rtlsdrDriver::open(int device_index) {
    mDeviceId = device_index;
    ...
    if ((device_index < 0) || (device_index >= n_rtlsdr)) {
        std::cerr << "Error: must specify which USB device to use with --device-index"
                  << std::endl;
...
static void *aux_thread(void *arg) {
    ...
    if (OpenRFSenseContext::getInstance()->getStartTimeSampling() != 0) {
        ...
        std::cout << "CurrentTime: " << currentTime.tv_sec << "." << currentTime.tv_nsec
                  << std::endl;
        std::cout << "StartTime Sampling: " << ref_time.tv_sec << "." << ref_time.tv_nsec
                  << std::endl;
    ...
    while (1) {
        ...
        if (last_gain != current_gain) {
            ...
            if (current_gain > 0) {
                ...
                if (r < 0) {
                    ...
                    if (r < 0) {
                        std::cerr << "ERROR: Failed to set manual tuner gain"
                                  << std::endl;
                    } else {
        ...
        if (last_fc != current_fc) {
            ...
            if (!cp.converter_enable) {
                ...
                if (r != 0)
                    std::cerr << "Error: unable to set center freq: "
                              << esense->getMinFreq() << std::endl;

            } else {
                // Converter

                if (!converterTune(
                        conv, esense->getMinFreq() / 1e3, &proxy_freq, &mustInvert)) {
                ...
                if (previous_proxy_freq != proxy_freq) {
                    ...
                    if (r != 0) {
                        ...
                        std::cout << "Center Frequency: " << proxy_freq * 1e3
                                  << std::endl;
                    }
                }
            }
    ...
    while (mRunning) {
        ...
        if (previous_freq != center_freq) {
            ...
            if (mConverterEnabled) {
                ...
                if (!converterTune(
                        &mConverterDriver, center_freq / 1e3, &proxy_freq, &mustInvert)) {
                    mRunning = false;
                    throw std::logic_error("Failed to converterTune");
                }
                ...
                if (previous_proxy_freq != proxy_freq) {
                    ...
                    if (r != 0) {
                        std::cerr << "Error: unable to set center frequency: "
                                  << proxy_freq * 1e3 << std::endl;
                        continue;
                    }
                ...
                if (OpenRFSenseContext::getInstance()->getDirectSamplingMode() > 0) {
                    if (center_freq >= 24e6 && direct_sampling) {
                        ...
                        if (rtlsdr_set_direct_sampling(mDevice, 0) < 0) {
                            std::cerr << "Error: unable to disable direct_sampling"
                                      << std::endl;
                            throw std::logic_error(
                                "Fatal Error, unable to disable direct_sampling");
                        }
                        ...
                        int r = rtlsdr_set_tuner_gain_mode(mDevice, 1);
                        if (r < 0) {
                            std::cerr << "ERROR: Failed to enable manual gain mode"
                                      << std::endl;
                            throw std::logic_error("Fatal Error");
                        }
                        r = rtlsdr_set_tuner_gain(mDevice, gain * 10);
                        ...
                        if (rtlsdr_set_direct_sampling(mDevice, 2) < 0) {
                            std::cerr << "Error: unable to set direct_sampling"
                                      << std::endl;
                            throw std::logic_error(
                                "Fatal Error, unable to set direct_sampling");
                        }
                ...
                if (r != 0) {
                    std::cerr << "Error: unable to set center frequency: " << center_freq
                              << std::endl;
                    continue;
                }
        ...
        if (fft_size != current_fft_size) {
            ...
            slen =
                ((current_fft_size - OpenRFSenseContext::getInstance()->getSoverlap()) *
        ...
        for (unsigned int i = 0; i < OpenRFSenseContext::getInstance()->getAvgFactor();
             i++) {
